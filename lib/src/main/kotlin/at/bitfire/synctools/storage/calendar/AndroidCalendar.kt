...
class AndroidCalendar(
    ...
    fun addEvent(entity: Entity, batch: CalendarBatchOperation) {
        ...
        val builder = CpoBuilder.newInsert(eventsUri).withValues(entity.entityValues)
        batch += builder
    ...
    fun numInstances(eventId: Long): Int? {
        ...
        * 1. expands the instances for the requested window into the Instances table,
        * 2. queries the Instances table with the given query.
        * So we request only the time window of the actual event to avoid unnecessary
        * instance expansion. */
        val windowStart = first
        val windowEnd = last

        val instancesUri = Instances.CONTENT_URI.asSyncAdapter(account)
            .buildUpon()
            .appendPath(windowStart.toString())       // begin timestamp
            .appendPath(windowEnd.toString())        // end timestamp
            .build()
        ...
        logger.info("Querying instances between $windowStart and $windowEnd and filtering for event IDs: $eventIdsSql")
