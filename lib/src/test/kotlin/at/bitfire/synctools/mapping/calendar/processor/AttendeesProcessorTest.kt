...
class AttendeesProcessorTest {
    ...
    fun `Attendee is email address`() {
        ...
        assertEquals(URI("mailto:attendee@example.com"), result.getProperty<Attendee>(Property.ATTENDEE).calAddress)
    }
    ...
    fun `Attendee is other URI`() {
        ...
        val result = VEvent(/* initialise = */ false)
        processor.process(entity, entity, result)
        assertEquals(URI("https://example.com/principals/attendee"), result.getProperty<Attendee>(Property.ATTENDEE).calAddress)
    }
    ...
    fun `Attendee is email address with other URI`() {
        ...
        val result = VEvent(/* initialise = */ false)
        processor.process(entity, entity, result)
        assertEquals(1, result.getProperties<Attendee>(Property.ATTENDEE).size)
