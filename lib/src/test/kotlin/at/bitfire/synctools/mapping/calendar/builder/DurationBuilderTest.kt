...
class DurationBuilderTest {
    ...
    fun `Recurring all-day event (with DTEND before START)`() {
        val result = Entity(ContentValues())
        val event = VEvent(propertyListOf(
            DtStart<Temporal>("20251017"),
            DtEnd<Temporal>("20251010"),    // DTEND before DTSTART should be ignored
            RRule<Temporal>("FREQ=DAILY;COUNT=5")
        ))
        builder.build(event, event, result)
    ...
    fun `Recurring all-day event (with DTEND equals START)`() {
        ...
        val event = VEvent(propertyListOf(
            DtStart<Temporal>("20251017"),
            DtEnd<Temporal>("20251017"),    // DTEND equals DTSTART should be ignored
            RRule<Temporal>("FREQ=DAILY;COUNT=5")
        ))
        builder.build(event, event, result)
        // default duration for all-day events: one day
    ...
    fun `Recurring non-all-day event (with DTEND)`() {
        ...
        val event = VEvent(propertyListOf(
            DtStart(LocalDateTime.parse("2025-10-10T01:02:03").atZone(tzVienna.toZoneId())),
            DtEnd(LocalDateTime.parse("2025-10-11T02:03:04").atZone(tzVienna.toZoneId())),
            RRule<Temporal>("FREQ=DAILY;COUNT=5")
        ))
        builder.build(event, event, result)
    ...
    fun `Recurring non-all-day event (with DTEND before DTSTART)`() {
        ...
        val event = VEvent(propertyListOf(
            DtStart(LocalDateTime.parse("2025-10-10T01:02:03").atZone(tzVienna.toZoneId())),
            DtEnd(LocalDateTime.parse("2025-10-10T00:02:03").atZone(tzVienna.toZoneId())),   // DTEND before DTSTART should be ignored
            RRule<Temporal>("FREQ=DAILY;COUNT=5")
        ))
        builder.build(event, event, result)
    ...
    fun `Recurring non-all-day event (with DTEND equals DTSTART)`() {
        ...
        val event = VEvent(propertyListOf(
            DtStart(LocalDateTime.parse("2025-10-10T01:02:03").atZone(tzVienna.toZoneId())),
            DtEnd(LocalDateTime.parse("2025-10-10T01:02:03").atZone(tzVienna.toZoneId())),   // DTEND equals DTSTART should be ignored
            RRule<Temporal>("FREQ=DAILY;COUNT=5")
        ))
        builder.build(event, event, result)
        // default duration for non-all-day events: zero seconds
    ...
    fun `alignWithDtStart (DTSTART all-day, DURATION all-day)`() {
        assertEquals(
            ...
            builder.alignWithDtStart(Period.ofDays(1), DtStart(LocalDate.now()))
        )
    }
    ...
    fun `alignWithDtStart (DTSTART non-all-day, DURATION all-day)`() {
        assertEquals(
            ...
            builder.alignWithDtStart(Period.ofDays(1), DtStart(LocalDateTime.now()))
        )
    }
    ...
    fun `alignWithDtStart (DTSTART all-day, DURATION non-all-day)`() {
        assertEquals(
            Period.ofDays(1),       // may not be 24 hours (for instance on DST switch)
            builder.alignWithDtStart(java.time.Duration.ofHours(25), DtStart(LocalDate.now()))
        )
    }
    ...
    fun `alignWithDtStart (DTSTART non-all-day, DURATION non-all-day)`() {
        assertEquals(
            java.time.Duration.ofDays(1),   // exactly 24 hours
            builder.alignWithDtStart(java.time.Duration.ofHours(24), DtStart(LocalDateTime.now()))
        )
    }
    ...
    fun `calculateFromDtEnd (dtStart=DATE, DtEnd=DATE)`() {
        val result = builder.calculateFromDtEnd(
            DtStart<Temporal>("20240328"),
            DtEnd<Temporal>("20240330")
        )
        assertEquals(
            Period.ofDays(2),
            result
    ...
    fun `calculateFromDtEnd (dtStart=DATE, DtEnd=DATE-TIME)`() {
        val result = builder.calculateFromDtEnd(
            DtStart<Temporal>("20240328"),
            DtEnd(LocalDateTime.parse("2024-03-30T12:34:12").atZone(tzVienna.toZoneId()))
        )
        assertEquals(
            Period.ofDays(2),
            result
    ...
    fun `calculateFromDtEnd (dtStart=DATE-TIME, DtEnd before dtStart)`() {
        val result = builder.calculateFromDtEnd(
            DtStart(LocalDateTime.parse("2024-03-28T01:02:03").atZone(tzVienna.toZoneId())),
            DtEnd(LocalDateTime.parse("2024-03-28T00:00:00").atZone(tzVienna.toZoneId()))
        )
        assertNull(result)
    }
    ...
    fun `calculateFromDtEnd (dtStart=DATE-TIME, DtEnd=DATE-TIME)`() {
        val result = builder.calculateFromDtEnd(
            DtStart(LocalDateTime.parse("2024-07-28T01:02:03").atZone(tzVienna.toZoneId())),
            DtEnd<Temporal>("20240728T010203Z")     // GMT+1 with DST → 2 hours difference
        )
        assertEquals(
